                    ...
                    <div className="flex">
                      <div className="relative flex space-x-8">
                        {Categories.length > 0 && Categories.map((category, key) => {
                          return (
                            <button key={key} onClick={() => set_show_slide_bar(!slide_bar)} type="button"
                              ...
                              <div className="row-start-1 grid grid-cols-6 gap-y-10 gap-x-8 text-sm">
                                ...
                                {Categories.length > 0 && Categories.map((category) => {
                                  if(category.CategoriesArray){
                                    return (
                                      <div>
                                        {category.CategoriesArray.map((catArray, index) => {
                                          return (
                                            <div key={index}>
                                              <p className="font-medium text-gray-900">{catArray.CatName}</p>
                                              <ul className="mt-6 flex flex-col space-y-6">
                                                  {catArray.CategoriesArray && catArray.CategoriesArray.map( (name, key) => { 
                                                    return (
                                                      <li className="flow-root" key={key}>
